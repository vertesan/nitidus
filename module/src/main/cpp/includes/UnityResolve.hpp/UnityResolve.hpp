/*
 * Modified by Vertesan under the terms of MIT License.
 */
  ...
  public:
    ...
    static auto Invoke(const std::string &funcName, Args... args) -> Return {
        ...
        if (address_.find(funcName) == address_.end() || !address_[funcName]) {
            auto xdlAddr = xdl_sym(hmodule_, funcName.c_str(), nullptr);
            if (!xdlAddr) {
    ...
    static auto FillClassMember(Class* klass, void* pKlass) -> void {
        ForeachFields(klass, pKlass);
        ForeachMethod(klass, pKlass);
    }
